...
각 문제별로 제어 출력 토픽과 운영 형태가 다르므로 코드/launch를 수정할 때 반드시 확인할 것.

| 문제 | 패키지   | 제어 출력 토픽 | 메시지 타입           | 운영 형태                                              |
| ---- | -------- | -------------- | --------------------- | ------------------------------------------------------ |
| 1-1  | pkg_p1_1 | `/cmd_vel`     | `geometry_msgs/Twist` | 실차 (모션캡쳐) — Docker 미포함, 로컬에서 colcon 빌드  |
| 1-2  | pkg_p1_2 | `/Accel`       | `geometry_msgs/Accel` | 시뮬레이터 — Docker 미포함                             |
| 2    | pkg_p2   | `/Accel`       | `geometry_msgs/Accel` | 시뮬레이터 — Docker 미포함                             |
| 3    | pkg_p3   | `/cmd_vel`     | `geometry_msgs/Twist` | 실차 (CAV마다 별도 clone) — **이 Dockerfile의 유일한 빌드 대상** |

> 시뮬레이터는 `/Accel`로 제어 입력을 받고, 실차(모션캡쳐) 환경은 `/cmd_vel`을 받는다.

## Dockerfile 빌드 범위

이 이미지는 **문제 3 (`pkg_p3`) 전용**이다. `Dockerfile`은 `pkg_p3`와 `domain_bridge`만 빌드하며, `entrypoint.sh`는 `ros2 launch pkg_p3 competition.launch.py`만 실행한다. 다른 문제(1-1, 1-2, 2)는 컨테이너 밖에서 직접 colcon build로 돌린다.

## 문제 3 — per-CAV clone 운영 가이드

4대 CAV가 각자 다른 도메인에서 단일 노드를 실행한다. CAV마다 레포를 clone한 뒤 아래 파일들만 수정하면 된다.

| 수정 대상                                                   | 의미                                        |
| ----------------------------------------------------------- | ------------------------------------------- |
| `ROS_DOMAIN_ID` (컨테이너 env)                              | 해당 CAV가 사용할 도메인 (1/2/3/4)          |
| `pkg_p3/launch/competition.launch.py` 의 활성 Node          | 본인 CAV 노드 1개만 살림 (현재 cav1만 활성) |
| `pkg_p3/launch/competition.launch.py` 의 `bridge_yaml_cavN` | 본인 CAV용 bridge yaml 선택                 |
| 본인 CAV용 `pkg_p3/config/bridge_cavN.yaml`                 | `to_domain`이 본인 도메인과 일치하는지 확인 |

> `p3.py`는 `ROS_DOMAIN_ID` 환경변수로 자신의 `vid`를 결정하므로 코드 자체를 수정할 필요는 없다. waypoint(`3_cavN.json`)와 conflict map(`3_zone_database.json`)도 path key (`path1`~`path4`) 분기로 자동 선택된다.
...
## 컨테이너 실행 (문제 3, per-CAV)

`ROS_DOMAIN_ID`만 본인 CAV에 맞춰 바꿔서 실행한다.

```bash
# CAV1
docker run --rm --net=host --ipc=host -e ROS_DOMAIN_ID=1 smyd:jetson
# CAV2
docker run --rm --net=host --ipc=host -e ROS_DOMAIN_ID=2 smyd:jetson
# CAV3
docker run --rm --net=host --ipc=host -e ROS_DOMAIN_ID=3 smyd:jetson
# CAV4
docker run --rm --net=host --ipc=host -e ROS_DOMAIN_ID=4 smyd:jetson
```

> 문제 1-1, 1-2, 2는 이 이미지로 실행할 수 없다. 해당 패키지는 컨테이너 밖에서 `colcon build` 후 `ros2 launch ...`로 직접 실행한다.
